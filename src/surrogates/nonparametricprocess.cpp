...
#include "surrogates/gaussian_process_ml.hpp"
#include "surrogates/gaussian_process_normal.hpp"
#include "surrogates/student_t_process_jef.hpp"
#include "surrogates/student_t_process_nig.hpp"
...
namespace bayesopt
{
  ...
  NonParametricProcess::NonParametricProcess(size_t dim, Parameters parameters, 
					     ...
					     MeanModel& mean,
					     randEngine& eng):
    mData(data), dim_(dim), mMean(mean), mSigma(parameters.sigma_s), mNoise(parameters.noise)
  {
    mUT.reset(new IsotropicUnscentedTransform(dim, parameters));
    mUT->setProcess(this);
  }
  ...
  double NonParametricProcess::getIntegratedOutcome(const vectord& query){
    return mUT->integratedOutcome(query);
  }
