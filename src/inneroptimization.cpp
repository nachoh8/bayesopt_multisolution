...
namespace bayesopt
{
  ...
  double NLOPT_Optimization::localTrialAround(vectord& Xnext)
  {
    ...
    for (size_t i = 0; i < n; ++i)
      {
	if (Xnext(i) < mDown[i]+radius || Xnext(i) > mUp[i]-radius)
	  {
    ...
    FILE_LOG(logDEBUG) << "Running inner trial nlopt_bobyqa with " << Xnext;
  ...
  double NLOPT_Optimization::run(vectord &Xnext, bool perDimension)
  {
    assert(mDown.size() == Xnext.size());
    assert(mUp.size() == Xnext.size());
    ...
    FILE_LOG(logDEBUG) << "Running nlopt_" << alg << " with " << Xnext;
	...
	try
	  {
	FILE_LOG(logDEBUG) << "Running nlopt_bobyqa with " << Xnext;
	...
	FILE_LOG(logDEBUG) << "2nd opt " << maxf2 << "-> " << Xnext
	  ...
	  {
	    FILE_LOG(logDEBUG) << "Local problem. Try alternative method.";
