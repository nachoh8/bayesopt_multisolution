...
namespace bayesopt
{
    ...
    void BOptState::loadOrSave(utils::FileParser &fp){
        ...
        fp.readOrWrite("mParameters.n_final_samples", mParameters.n_final_samples);
        fp.readOrWrite("mParameters.n_parallel_samples", mParameters.n_parallel_samples);
        ...
        if(fp.isReading()){
            ...
            mParameters.par_type = str2par(fp.read("mParameters.par_type").c_str());
        }
            ...
            fp.write("mParameters.par_type", par2str(mParameters.par_type));
        }
        
        fp.readOrWrite("mParameters.burnout_mcmc_parallel", mParameters.burnout_mcmc_parallel);
        fp.readOrWrite("mParameters.l_all", mParameters.l_all);
        fp.readOrWrite("mParameters.use_spsa", mParameters.use_spsa);
        fp.readOrWrite("mParameters.epsilon", mParameters.epsilon);
        fp.readOrWrite("mParameters.force_jump", mParameters.force_jump);
        fp.readOrWrite("mParameters.active_hyperparams_coef", mParameters.active_hyperparams_coef);
        ...
        fp.readOrWrite("mParameters.kernel.domain_center", mParameters.kernel.domain_center);
        fp.readOrWrite("mParameters.kernel.domain_std", mParameters.kernel.domain_std);      
        ...
        fp.readOrWrite("mParameters.unscented_alpha", mParameters.unscented_alpha);
        fp.readOrWrite("mParameters.unscented_beta", mParameters.unscented_beta);
        fp.readOrWrite("mParameters.unscented_kappa", mParameters.unscented_kappa);
        fp.readOrWrite("mParameters.input_noise_variance", mParameters.input_noise_variance);
        fp.readOrWrite("mParameters.use_unscented_criterion", mParameters.use_unscented_criterion);

        fp.readOrWrite("mY", mY);
        fp.readOrWrite("mX", mX);
        fp.readOrWrite("mDebugBestPoints", mDebugBestPoints);

        fp.readOrWrite("mParameters.num_solutions", mParameters.num_solutions);
        fp.readOrWrite("mParameters.diversity_level", mParameters.diversity_level);
    }
} //namespace bayesopt
