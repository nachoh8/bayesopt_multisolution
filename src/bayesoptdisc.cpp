...
namespace bayesopt
{
  ...
  {
    mParameters.use_spsa = 0; // Only for continuous optimization
  ...
  // vectord DiscreteModel::getLastPoint()
  // {
  //   return getData()->getLastSampleX();
  // }
  ...
  void DiscreteModel::sampleCriteria(vectord &xNext)
  {
    FILE_LOG(logDEBUG) << "sampleCriteria() not implemented yet in discrete, using findOptimal() instead";
    findOptimal(xNext);
  };

  void DiscreteModel::rejectionSamplingCriteria(vectord &xNext)
  {
    FILE_LOG(logDEBUG) << "rejectionSamplingCriteria() not implemented yet in discrete, using findOptimal() instead";
    findOptimal(xNext);
  };

  void DiscreteModel::parallelTemperingCriteria(vectord &xNext)
  {
    FILE_LOG(logDEBUG) << "parallelTemperingCriteria() not implemented yet in discrete, using findOptimal() instead";
    findOptimal(xNext);
  };
