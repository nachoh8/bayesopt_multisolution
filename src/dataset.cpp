...
namespace bayesopt
{
  ...
  Dataset::Dataset(const Dataset& dataset) {
    mMinIndex = dataset.mMinIndex;
    mMaxIndex = dataset.mMaxIndex;

    mY = vectord(dataset.mY.size());
    std::copy(dataset.mY.begin(), dataset.mY.end(), mY.begin());

    for (size_t i=0; i<dataset.mX.size(); ++i) {
      vectord x = vectord(dataset.mX[0].size());
      std::copy(dataset.mX[i].begin(), dataset.mX[i].end(), x.begin());
      mX.push_back(x);
    }
  }
