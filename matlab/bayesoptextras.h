...
static void struct_2d_array(const mxArray *s, const char *name, size_t *n, size_t *m, double *result);
...
void struct_2d_array(const mxArray *s, const char *name, size_t *n, size_t *m, double *result)
{
  mxArray *val = mxGetField(s, 0, name);
  if (val) 
    {
      if(!(mxIsNumeric(val) && !mxIsComplex(val)))
	{
	  mexErrMsgTxt("Param fields must be vector or matrix");
	}
      else
	{	   
	  *m = mxGetM(val);
	  *n = mxGetN(val);
	  memcpy(result, mxGetPr(val), *n * *m * sizeof(double));
	}
    }
  else
    {
...
{  
  ...
  size_t n_hp_test, n_coef_test, n_dim_domain, n_basis_kernels;

  bopt_params parameters = initialize_parameters_to_default();

  n_hp_test = parameters.kernel.n_hp;
  n_coef_test = parameters.mean.n_coef;
  n_basis_kernels = parameters.kernel.n_basis_kernels;
  n_dim_domain = parameters.kernel.n_dim_domain;
  ...
  struct_2d_array(params, "kernel_domain_center", &parameters.kernel.n_basis_kernels, 
	       &parameters.kernel.n_dim_domain, &parameters.kernel.domain_center[0][0]);
  struct_2d_array(params, "kernel_domain_std", &n_basis_kernels, 
	       &n_dim_domain, &parameters.kernel.domain_std[0][0]);


  CHECK0(parameters.kernel.n_hp == n_hp_test, 
	 "Error processing kernel parameters");

  CHECK0(parameters.kernel.n_basis_kernels == n_basis_kernels, 
	 "Error processing kernel parameters");

  CHECK0(parameters.kernel.n_dim_domain == n_dim_domain, 
	 "Error processing kernel parameters");
