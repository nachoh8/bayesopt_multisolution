...
#include "surrogates/conditionalbayesprocess.hpp"

#include "distributions/joint_prob_distribution.hpp"
...
namespace bayesopt
{
  ...
  public:
    ...
    /**
     * \brief Function that returns the prediction of the GP for multiple points
     * in the hypercube [0,1]
     * @param points that are contained in the hypercube [0,1] to evaluate the
     * Gaussian process
     * @return pointer to the joint distribution, containing the predicted
     * means and the covariance matrix.
     */
    JointDistribution* pointsPrediction(const vecOfvec &points);
  ...
  private:
    ...
    JointDistribution* jointd_;   // TODO(Javier): maybe a member its not required
  };
