...
namespace bayesopt  {
  ...
  public:
    ...
    /**
     * Checks if query is inside the bounds of the problem
     */
    bool checkBounds(const vectord &query);

    /** Finalize the optimization process with refinements */
    virtual void finalizeOptimization();

    /** Used by local optimization callback */
    double evaluatePolishSample(const vectord& x);
  ...
  protected:
     ...
     */
    void sampleCriteria(vectord &xNext);
    void rejectionSamplingCriteria(vectord &xNext);
    void parallelTemperingCriteria(vectord &xNext);
    ...
    /** Performs a local optimization to refine the solution */
    void polishOptimization(int numIters, double unitInterval = 0.05);
  ...
  private:
    ...
    boost::scoped_ptr<CritCallback> mSamplingCallback;
    
  public: // TODO (Javier): its public to allow access to utils/DisplayCrit plotting  
    boost::scoped_ptr<MCMCSampler> cSampler;
