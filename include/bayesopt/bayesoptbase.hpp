...
namespace bayesopt {
  ...
  public:
    ...
    virtual vectord evaluateSamples( const vecOfvec &queries ) {assert(false); return vectord(1);};
     ...
     *
     * @param bestPoint returns point with the optimum value in a
     * ublas::vector.
     */
    void optimize(vectord &bestPoint);

    /**  Execute ONE step of the optimization process. */
    void stepOptimization();

    /** Initialize the optimization process.  */
    void initializeOptimization();

    /** Finalize the optimization process with refinements */
    ...
    void setNormalizeCriteria(bool normalize);
    void startAnnealingCycle();
    void stepAnnealing();
    void setBoltzmannPolicy(bool activate, double temperature=1.0, double tempReduction=1.0, double freezingReduction=1.0);
    double modifyCriteria(double value, bool normalize=false, bool clamp=false, bool useBoltzmann=false, double temperature=1.0);
    double criteriaCorrection(double value);
    void setBurnOutSamples(int burnout){ mBurnOutSamples = burnout; }

  protected:
    /** Get optimal point in the inner space (e.g.: [0-1] hypercube) */
    vectord getPointAtMinimum(bool useMean = false);
    ...
    vectord evaluateSamplesInternal( const vecOfvec &query );
    ...
    /**
     * Sample the next point using the criteria distribution
     */
    virtual void sampleCriteria(vectord &xNext) = 0;
    virtual void rejectionSamplingCriteria(vectord &xNext) = 0;
    virtual void parallelTemperingCriteria(vectord &xNext) = 0;
    ...
    /** Eases the Save functionality. Saves only if required by the parameters */
    void saveOptimizationIfRequired();

    /**
     *  \brief Generates a sample from a non evaluated sample, is a sample that
     *  its point (x) is known but its result (y) is not available.
     *  Returns true if an non evaluated sample was generated, false otherwise.
     */
    bool generateNonEvaluatedSample();
  ...
  protected:
    ...
    // TODO(Javier): mcmc criteria sampling
    int mBurnOutSamples;
    bool mNormalizeCriteria;
    bool mUseBoltzmannPolicy;    // simmulated anneling variables
    double mInitTemperature;

    double mCurrentTemperature;  // current sim annealing temperature
    double mTempReduction;

    double mFreezingTemperature; // lower bounds of annealing, updated at each cycle
    double mFreezingReduction;

    vecOfvec mLastMCMCParticles;
    vecOfvec mLastMCMCSamples;

    vecOfvec mDebugBestPoints; // Best point at each iteration

  private:
    double mYPrev;
    size_t mCounterStuck;
    boost::scoped_ptr<SPSA> mSPSA_sampler;
    boost::scoped_ptr<IntegratedResponse> mUT;
    size_t mIntegratedIndex;

  private:

    BayesOptBase();

    double evaluateAndAddNextPoint(const vectord& xNext, bool retrain);
    bool checkStuck(double yNext);
  ...
  protected: // multisolution
    vecOfvec centroids;
    // vectord max_dist_centroid;
    vecOfvec cluster_lb, cluster_ub;
    size_t current_cluster;
    bool cluster_sampling = false;
  };
