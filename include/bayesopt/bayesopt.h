...
extern "C" {
   ...
   typedef void (*multi_eval_func)(unsigned int n, unsigned int batch_size, const double *x,
               double* res,
			      double *gradient, /* NULL if not needed */
			      void *func_data);
...
/** 
 * @brief C wrapper for the Parallel Bayesian optimization algorithm. 
 * This function assumes continuous optimization.
 * 
 * @param nDim number of input dimensions
 * @param f pointer to the function to optimize
 ...
 * @return error code
 */
  BAYESOPT_API int parallel_bayes_optimization(int nDim, multi_eval_func f, void* f_data,
				      const double *lb, const double *ub,
				      double *x, double *minf,
				      bopt_params parameters);
 ...
 * This function assumes continuous optimization. It logs the result
 * in a simple format.
 * 
 * @param nDim number of input dimensions
 * @param f pointer to the function to optimize
 ...
 * @return error code
 */
  BAYESOPT_API int bayes_optimization_log(int nDim, eval_func f, void* f_data,
					  const double *lb, const double *ub,
					  double *x, double *minf,
					  const char* filename, 
					  bopt_params parameters);
