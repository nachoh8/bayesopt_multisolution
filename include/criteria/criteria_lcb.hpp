...
namespace bayesopt
{
  ...
  class NormalizedConfidenceBound: public Criteria
  {
  public:
    virtual ~NormalizedConfidenceBound(){};
    void init(NonParametricProcess* proc)
    { 
      mProc = proc;
      mBeta = 1.0;
    ...
    double operator() (const vectord &x) 
    { 
      double value = mProc->prediction(x)->lowerConfidenceBound(mBeta); 
      
      double maxY = mProc->getData()->getValueAtMaximum ();
      double minY = mProc->getData()->getValueAtMinimum ();
      value = (value - maxY) / (minY - maxY);
      return -value; // Return negative value
    };
    std::string name() {return "cLCBnorm";};
  private:
    double mBeta;
  };
  
  class ExponentialConfidenceBound: public Criteria
  {
  public:
    virtual ~ExponentialConfidenceBound(){};
    void init(NonParametricProcess* proc)
    { 
      mProc = proc;
      mBeta = 1.0;
    ...
    { 
      ...
      double exp_value = exp(-value/mTemperature);
      return -exp_value;
    };
    std::string name() {return "cLCBexp";};
  private:
    double mBeta;
    double mTemperature;
  };
