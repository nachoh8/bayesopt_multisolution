from .rover_utils import RoverDomain, PointBSpline, ConstObstacleCost, NegGeom, AABoxes, PixelMask, UnionGeom, AdditiveCosts, \
    ConstCost, plot_2d_rover
import numpy as np
import matplotlib.pyplot as plt

from .ebo_helper import ConstantOffsetFn, NormalizedInputFn
from .rover_maps import levels_from_image, masks_from_image

def create_cost_from_image(filename):
    levels, _, img_shape = levels_from_image(filename)
    masks, _, _ = masks_from_image(filename)

    pixel_size = 1./(img_shape[0]-1) # Assume is box
    def get_coord(point):
        return [p * pixel_size + pixel_size*0.5 for p in point]
    ...
    hard_obstacles = np.array(levels[-1])
    hard_obstacles = np.array([get_coord(p) for p in hard_obstacles])
    trees = get_boxes(hard_obstacles)
    
    #trees = PixelMask(masks[-1], r_l[0], r_h[0])

    soft_obstacles = None
    hills = None
    if len(levels)==3:
        soft_obstacles = np.array(levels[1])
        soft_obstacles = np.array([get_coord(p) for p in soft_obstacles])
        hills = get_boxes(soft_obstacles)
        #hills = PixelMask(masks[1], r_l[0], r_h[0])
    ...
    goal = np.array([0.6, 0.9])#np.array([0.95, 0.95])

    costs = [ConstObstacleCost(obstacles, cost=20.), ConstCost(0.05)]

    if hills is not None:
        costs.append(ConstObstacleCost(hills, cost=1.))
...
def create_small_domain(n_points=3, extra_domain=0.1, img_map=None):
    if img_map is None:
        cost_fn, start, goal = create_cost_small()
    else:
        cost_fn, start, goal = create_cost_from_image(img_map)
    ...
    domain = RoverDomain(cost_fn,
                         ...
                         goal=goal,
                         traj=traj,
                         s_range=np.array([[-extra_domain, -extra_domain], [1.0+extra_domain, 1.0+extra_domain]]))
...
class RoverProblem():
    def __init__ (self, num_points, k=3, s=None, extra_domain=0.1, img_map=None):
        self.num_points = num_points
        
        self.domain = create_small_domain(num_points, extra_domain=extra_domain, img_map=img_map)

        self.domain.traj.set_extra_params(extra={'k': k, 's': s})

        raw_x_range = np.repeat(self.domain.s_range, num_points, axis=1)
        self.f = NormalizedInputFn(self.domain, raw_x_range)

        self.lb = self.f.get_range()[0]
        self.ub = self.f.get_range()[1]
...
def main():
    ...
    from rover_utils import plot_2d_map, plot_2d_trajectory
    img_filename = './hill_map.png'
    rover = RoverProblem(2, k=2, s=0.0, extra_domain=0.0, img_map=img_filename)

    #x = [0.32, 0.3, 0.1, 0.6]
    #x = [0.3392, 0.1873, 0.883984, 0.519581]
    x = [0.2652, 0.4407, 0.1450, 0.5485]
    
    print('Input = {}'.format(x))
    print('Output = {}'.format(rover(x)))

    fig, ax = plt.subplots(1,1)

    plot_2d_map(ax, rover.domain)

    num_trajs = 20
    samples = np.random.normal(x, np.sqrt(0.02**2), (num_trajs, len(x)))
    samples_y = []
    for i in range(num_trajs):
        samples_y.append(rover(samples[i, :]))
        plot_2d_trajectory(ax, rover.domain, alpha=0.5, color='b')

    y = rover(x)
    plot_2d_trajectory(ax, rover.domain, color='r')

    ax.set_title('y: {} - avg: {}'.format(y, np.mean(samples_y)))

    #x = [0.29,0.13,0.87,0.6]
    x = [0.352430, 0.1524, 0.8872, 0.5619] # default rep 0
