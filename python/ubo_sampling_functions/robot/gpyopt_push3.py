...
class RobotSim(object):
    def __init__(self, dim):
        ...
        self.lb = np.array([-5., -5. ,1.])
        self.ub = np.array([5., 5. ,30.])
        
        if self.dim > 3:
	        self.lb = np.append(self.lb, 0)
	        self.ub = np.append(self.ub, 2 * np.pi)
    ...
    def call(self, X_test):
        try:
            return self._call(X_test)
        except:
            print (traceback.format_exc())

    def _call(self, X_test):
        rx = X_test[0]
        ry = X_test[1]
        gx = self.gx
        gy = self.gy
        ...
        if ret[0] != ret[0] or ret[1] != ret[1]: # check for NaN
            ret = np.array([0,0]) # initial position

        return np.linalg.norm(np.array([gx, gy]) - ret)
    ...
    for iteration in xrange(repetitions):
        ...
        while not all_ok and watchdog < 3:
            try:
                ...
                print ("Optimum:" + str(myProblem.fx_opt))
                if values is None:
