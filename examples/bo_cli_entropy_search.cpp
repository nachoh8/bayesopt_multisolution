...
{
  std::cout << "ENTROPY SEARCH" << std::endl;
  ...
  try
	...
	("iterations,i", po::value<size_t>(&n_iter)->default_value(200), "number of iterations")
  ...
  for(size_t repetition = init_rep; repetition < init_rep+n_simulations; repetition++)
  {
    ...
    set_criteria(&par, "cEntropySearch");
    par.n_crit_params = 4;
    par.crit_params[0] = 20;
    par.crit_params[1] = 500;
    par.crit_params[2] = 10;
    par.crit_params[3] = 500;
    
    std::stringstream sstm;
    sstm << "output/" << "entropysearch_" << problem_name <<  "_rep_" << repetition << ".dat";
    std::string datfile = sstm.str();
    strcpy(par.save_filename, datfile.c_str());
    par.load_save_flag = 2;
    par.force_jump = 0; // TODO: model gets updated twice when jump is forced
    ...
    std::cout << "STEP OPTIMIZATION STARTS NOW!" << std::endl;
