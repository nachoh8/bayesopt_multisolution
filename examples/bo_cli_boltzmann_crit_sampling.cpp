...
int main(int nargs, char *args[])
{
  ...
  for(int i=-1; i<3; i++)
  {
    temperatures.push_back(std::pow(10,i));
  }
  ...
  for(size_t repetition = init_rep; repetition < init_rep+n_simulations; repetition++)
  {
    ...
    sstm << "output/" << "fixedtemp_mcmc_" << problem_name << "_at_" << temp << "_rep_" << repetition << ".dat";
    ...
    // TODO(Javier): mcmc criteria sampling, LCB as negative
    optimizer->setNormalizeCriteria(true);
    optimizer->setBoltzmannPolicy(true, temp, 1, 1);
    name = "ei_boltz_critsampl_" + name;
