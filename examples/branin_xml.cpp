        ...
        while (getline( input, currentLine)){
            if(currentLine.length() > 0){
                ...
                while(prefix_index != std::string::npos){
                    ...
                    while(number_index < currentLine.size() && 
			  std::isdigit(currentLine.at(number_index)))
		      {
                    ...
                    if(x_index >= xin.size()){
                        ...
                        std::cout << "ERROR: parameter " << x_index 
...
public:
  ...
  double evaluateSample( const vectord& xin)
  {
    ...
    // Call python script
    std::string call = "python ../examples/standalone_calls/eval_branin_xml.py " 
      + created_filename + " " + results_filename;

    // Hope nobody do something dirty on the Python modules.
    int foo = std::system(call.c_str());
